...
function longLeadtimeItems() {
        ...
        if (item[15] == '') {
          if (inYear - reqYear == 1) {
            if (12 - reqMonth + inMonth > 1) {
              ...
              waitingForPrint.push(printToArrive);
            } else if (inYear - reqYear == 0) { // Ordered in the same year
              if (inMonth - reqMonth > 1) {
                Logger.log('Waiting for print for over 1 month at row %d', i+129);
                let printToArrive = {
  ...
  <ol>\
  <li>Scanning Vendor Payment Date remians empty for at least one month or</li>\
  ...
  GmailApp.sendEmail('zl37@nyu.edu', 'Long Waiting CDL Orders', 'Placeholder text', {
    from: 'zl37@nyu.edu',
...
function tableConstruct(checklist, checkType) {
  ...
  <th style="width:120px">Order Num</th>\
